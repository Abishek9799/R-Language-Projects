...
```{r}
selectInput("d1Year", label = h4("Year 1"), 
    choices = c(2012:2018), selected = c(2012))
selectInput("d2Year", label = h4("Year 2"), 
    choices = c(2012:2018), selected = c(2013))
...
Traffic Accidents by Light conditions and Age Category {data-orientation=rows}
=====================================  
...
```{r}
radioButtons("Age", label = h4("Age Category"),
    choices = c("Adult", "Young Adult", "Senior","Youth"), selected = c("Adult"))

selectInput("Light_Conditions", label = h4("Light Conditions"), 
    choices = c("Daylight","Dusk", "Dark Lighted","Dark Unknown Lighting","Dawn","Dark Not Lighted","51"), selected = c("Daylight"))
...
renderValueBox({
  d2 <-
    ...
    filter( age.cat %in% input$Age, 
            Lightcondition %in% input$Light_Conditions)
...
Traffic Accidents by Weather, time of day and Violation {data-orientation=rows}
=====================================  
...
```{r}
radioButtons("Time_of_Day", label = h4("Time of Day"), 
    choices = as.character(unique(dat$time.of.day)), selected = c("Morning Commute"))
radioButtons("weather", label = h4("Weather"), 
    choices =  as.character(unique(dat$Weather)), selected = c("Clear"))
selectInput("d1Violation", label = h4("Driver 1 Violation"),
    choices = as.character(unique(dat$Violation1_Drv1)), selected = c("Inattention Distraction"))
selectInput("d2Violation", label = h4("Driver 2 Violation"),
    choices = as.character(unique(dat$Violation1_Drv2)), selected = c("No Improper Action"))
...
renderValueBox({
  d2 <-
    dat %>%
    filter( time.of.day %in% input$Time_of_Day, 
            Weather %in% input$weather, 
            Violation1_Drv1 %in% input$d1Violation, 
            Violation1_Drv2 %in% input$d2Violation )
...
This Dashboard exercises the Tempe Crash data from Arizona Department of Transportation. 

The dataset can be accessed from [**City of Tempe Open Data Portal**](https://data.tempe.gov/).

This data includes vehicle/vehicle, vehicle/bicycle and vehicle/pedestrian crashes in Tempe. The data also includes the type of crash and location. The dashboard is designed to give an insights into various factors contributing to accidents in the Tempe city.

<br>

### Dashboard Author

**Abishek Rajaswamy Kuppusamy**, A Passionate Engineer with a combined focus on Statistics and Data Science has experience in designing machine learning models and coming up with innovative ideas to solve unstructured business problems.

* [Email](mailto:arajaswa@asu.edu)

* [Connect via LinkedIn](https://www.linkedin.com/in/abishekrk/)
